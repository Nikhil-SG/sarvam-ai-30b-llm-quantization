#!/usr/bin/env python3
"""
MxMoE Pipeline — Clean orchestrator.

Replaces the legacy mxmoe orchestrator.

Usage (from project root):
    python -m pipelines.mxmoe.pipeline                           # All modules
    python -m pipelines.mxmoe.pipeline --module 1                # Sensitivity only
    python -m pipelines.mxmoe.pipeline --module 1 2              # Profile + Synthesise
...
from pipelines.mxmoe.modules import MODULE_MAP, MODULE_NAMES


def main() -> None:
    parser = argparse.ArgumentParser(
        description="MxMoE Mixed-Precision Pipeline — sarvamai/sarvam-30b"
    )
    parser.add_argument(
        "--module", nargs="+", type=int, default=None,
        help="Module number(s) to run (1-5). Default: all.",
    )
    parser.add_argument(
        "--config", type=str, default="configs/mxmoe.yaml",
    ...
    logger = get_logger("mxmoe.pipeline")
    logger.info("=" * 70)
    logger.info("MxMoE Mixed-Precision Pipeline — sarvamai/sarvam-30b")
    logger.info("=" * 70)
    get_device_info()

    modules: List[int] = args.module or [1, 2, 3, 4, 5]
    ...
    for num in modules:
        ...
        scope_config_to_mxmoe_module(config, num)
        setup_logger(f"mxmoe_module_{num}", log_dir=config.output.logs_dir)
        ...
        try:
            ...
            if not result["status"].startswith("✗"):
                test_passed = run_module_tests(
                    num, config, logger, pipeline_name="mxmoe"
                )
                if not test_passed:
